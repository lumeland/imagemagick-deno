...
export class PixelCollection extends NativeInstance
  ...
  setArea(
    ...
    width: number,
    height: number,
    quantumPixelsOrNumberPixels: quantumArray | number[],
  ): void {
    Exception.usePointer((exception) => {
      const pixels = (quantumPixelsOrNumberPixels instanceof Uint8Array)
        ? quantumPixelsOrNumberPixels
        : new Uint8Array(quantumPixelsOrNumberPixels);
      _withQuantumArray(pixels, (pixelsPtr) => {
        ImageMagick._api._PixelCollection_SetArea(
          this._instance,
          x,
          y,
          width,
