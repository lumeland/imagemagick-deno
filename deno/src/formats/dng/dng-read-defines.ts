...
import { DefinesCreator } from "../../defines/defines-creator.ts";
import { DngOutputColor } from "./dng-output-color.ts";
import { IDefine } from "../../defines/define.ts";
  ...
  getDefines(): IDefine[] {
    ...
    if (this.hasValue(this.disableAutoBrightness)) {
      defines.push(
        this.createDefine(
          "no-auto-bright",
          this.disableAutoBrightness as boolean,
        ),
      );
    }
    ...
    if (this.hasValue(this.outputColor)) {
      defines.push(
        this.createDefine("output-color", this.outputColor as number),
      );
    }
    ...
    if (this.hasValue(this.useCameraWhitebalance)) {
      defines.push(
        this.createDefine(
          "use-camera-wb",
          this.useCameraWhitebalance as boolean,
        ),
      );
    }
    ...
    if (this.hasValue(this.useAutoWhitebalance)) {
      defines.push(
        this.createDefine("use-auto-wb", this.useAutoWhitebalance as boolean),
      );
    }
