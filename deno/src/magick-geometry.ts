...
import { GeometryFlags } from "./internal/geometry-flags.ts";
...
export class MagickGeometry {
    ...
    if (typeof widthOrValueOrX === "number") {
      ...
      try {
        _withString(widthOrValueOrX, (valuePtr) => {
          const flags = ImageMagick._api._MagickGeometry_Initialize(
            instance,
            valuePtr,
          );
  ...
  private initialize(instance: number, flags: number) {
    ...
    this._ignoreAspectRatio = this.hasFlag(
      flags,
      GeometryFlags.IgnoreAspectRatio,
    );
