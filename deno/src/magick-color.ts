...
export class MagickColor {
    ...
    if (typeof colorOrRed === "string") {
      ...
      try {
        ...
        _withString(colorOrRed, (colorPtr) => {
          if (
            ImageMagick._api._MagickColor_Initialize(instance, colorPtr) === 0
          ) {
  ...
  toShortString(): string {
    ...
    return `#${this.toHex(this._r)}${this.toHex(this._g)}${
      this.toHex(this._b)
    }`;
  }
