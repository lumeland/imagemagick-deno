...
import { GifDisposeMethod } from "./gif-dispose-method.ts";
...
import { MorphologySettings } from "./settings/morphology-settings.ts";
...
import { _withByteArray, _withDoubleArray } from "./internal/native/array.ts";
...
export class MagickImage extends NativeInstance implements IMagickImage {
  ...
  get animationIterations(): number {
    return ImageMagick._api._MagickImage_AnimationIterations_Get(
      this._instance,
    );
  }
  set animationIterations(value: number) {
    ImageMagick._api._MagickImage_AnimationIterations_Set(
      this._instance,
      value,
    );
  }
  ...
  get animationTicksPerSecond(): number {
    return ImageMagick._api._MagickImage_AnimationTicksPerSecond_Get(
      this._instance,
    );
  }
  set animationTicksPerSecond(value: number) {
    ImageMagick._api._MagickImage_AnimationTicksPerSecond_Set(
      this._instance,
      value,
    );
  }
  ...
  get attributeNames(): ReadonlyArray<string> {
    ...
    let name = ImageMagick._api._MagickImage_GetNextAttributeName(
      this._instance,
    );
    while (name !== 0) {
      artifactNames.push(ImageMagick._api.UTF8ToString(name));
  ...
  get backgroundColor(): MagickColor {
    const colorPtr = ImageMagick._api._MagickImage_BackgroundColor_Get(
      this._instance,
    );
  ...
  set backgroundColor(value: MagickColor) {
    value._use((valuePtr) => {
      ImageMagick._api._MagickImage_BackgroundColor_Set(
        this._instance,
        valuePtr,
      );
    });
  ...
  set colorSpace(value: ColorSpace) {
    Exception.usePointer((exception) => {
      ImageMagick._api._MagickImage_ColorSpace_Set(
        this._instance,
        value,
        exception,
      );
  ...
  get endian(): number {
    return ImageMagick._api._MagickImage_Endian_Get(this._instance);
  }
  set endian(value: number) {
    ImageMagick._api._MagickImage_Endian_Set(this._instance, value);
  }
  ...
  get filterType(): number {
    return ImageMagick._api._MagickImage_FilterType_Get(this._instance);
  }
  ...
  get gamma(): number {
    return ImageMagick._api._MagickImage_Gamma_Get(this._instance);
  }

  get gifDisposeMethod(): GifDisposeMethod {
    return ImageMagick._api._MagickImage_GifDisposeMethod_Get(this._instance);
  }
  ...
  get interlace(): Interlace {
    return ImageMagick._api._MagickImage_Interlace_Get(this._instance);
  }
  ...
  get orientation(): OrientationType {
    return ImageMagick._api._MagickImage_Orientation_Get(this._instance);
  }
  ...
  get signature(): string | null {
    return Exception.usePointer((exception) => {
      return _createString(
        ImageMagick._api._MagickImage_Signature_Get(this._instance, exception),
      );
    });
  }
  ...
  get virtualPixelMethod(): VirtualPixelMethod {
    return Exception.usePointer((exception) => {
      return ImageMagick._api._MagickImage_VirtualPixelMethod_Get(
        this._instance,
        exception,
      );
    });
  ...
  set virtualPixelMethod(value: VirtualPixelMethod) {
    Exception.usePointer((exception) => {
      ImageMagick._api._MagickImage_VirtualPixelMethod_Set(
        this._instance,
        value,
        exception,
      );
  ...
  autoOrient(): void {
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_AutoOrient(
        this._instance,
        exception.ptr,
      );
  ...
  brightnessContrast(
    ...
    Exception.use((exception) => {
      ImageMagick._api._MagickImage_BrightnessContrast(
        this._instance,
        brightness.toDouble(),
        contrast.toDouble(),
        channels,
  ...
  charcoal(radiusOrUndefined?: number, sigmaOrUndefined?: number): void {
    ...
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_Charcoal(
        this._instance,
        radius,
        sigma,
        exception.ptr,
  ...
  clahe(
    xTiles: number | Percentage,
    yTiles: number | Percentage,
    numberBins: number,
    clipLimit: number,
  ): void {
    Exception.usePointer((exception) => {
      const xTilesValue = xTiles instanceof Percentage
        ? xTiles.multiply(this.width)
        : xTiles;
      const yTilesValue = yTiles instanceof Percentage
        ? yTiles.multiply(this.height)
        : yTiles;
      ImageMagick._api._MagickImage_Clahe(
        this._instance,
        xTilesValue,
        yTilesValue,
        numberBins,
  ...
  compare(
    ...
    return Exception.usePointer((exception) => {
      ...
      return ImageMagick._api._MagickImage_CompareDistortion(
        this._instance,
        image._instance,
        metric,
        channels,
  ...
  composite(
    image: IMagickImage,
    composeOrPoint?: CompositeOperator | Point,
    pointOrArgsOrChannels?: Point | string | Channels,
    channelsOrArgs?: Channels | string,
    channelsOrUndefined?: Channels,
  ): void {
    let x = 0;
    let y = 0;
    ...
    Exception.usePointer((exception) => {
      ImageMagick._api._MagickImage_Composite(
        this._instance,
        image._instance,
        x,
        y,
  ...
  compositeGravity(
    ...
    Exception.usePointer((exception) => {
      ImageMagick._api._MagickImage_CompositeGravity(
        this._instance,
        image._instance,
        gravity,
        x,
  ...
  contrastStretch(
    blackPoint: Percentage,
    whitePointOrUndefined?: Percentage,
    channelsOrUndefined?: Channels,
  ): void {
    const pixels = this.width * this.height;
    const lower = blackPoint.multiply(pixels);
    const upper = pixels -
      (whitePointOrUndefined ?? blackPoint).multiply(pixels);
    const channels = this.valueOrDefault(
      channelsOrUndefined,
      Channels.Undefined,
    );
  ...
  static create(
    fileNameOrArrayOrColorOrUndefined?: string | ByteArray | MagickColor,
  ...
  deskew(threshold: Percentage): number {
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_Deskew(
        this._instance,
        threshold.toQuantum(),
        exception.ptr,
      );
  ...
  distort(
    method: DistortMethod,
    settingsOrParams: number[] | DistortSettings,
    params?: number[],
  ): void {
    ...
    Exception.use((exception) => {
      _withDoubleArray(distortArgs, (distortArgsPtr: number) => {
        const instance = ImageMagick._api._MagickImage_Distort(
          this._instance,
          method,
          bestFit,
          distortArgsPtr,
  ...
  evaluate(
    channels: Channels,
    operatorOrGeometry: EvaluateOperator | MagickGeometry,
    valueOrPercentageOrOperator: number | Percentage | EvaluateOperator,
    valueOrPercentage?: number | Percentage,
  ): void {
    if (typeof operatorOrGeometry === "number") {
      const operator = operatorOrGeometry;
      const value = typeof valueOrPercentageOrOperator === "number"
        ? valueOrPercentageOrOperator
        : valueOrPercentageOrOperator.toQuantum();
      Exception.usePointer((exception) => {
        ImageMagick._api._MagickImage_EvaluateOperator(
          this._instance,
          channels,
          operator,
          value,
      ...
      const value = typeof valueOrPercentage === "number"
        ? valueOrPercentage
        : valueOrPercentage.toQuantum();
      ...
      Exception.usePointer((exception) => {
        MagickRectangle.use(this, geometry, (rectangle) => {
          ImageMagick._api._MagickImage_EvaluateGeometry(
            this._instance,
            channels,
            rectangle,
            operator,
  ...
  extent(
    geometryOrWidth: MagickGeometry | number,
    widthOrGravityOrBackgroundColor?: Gravity | MagickColor | number,
    backgroundColorOrGravity?: MagickColor | Gravity,
  ): void {
    ...
    if (geometryOrWidth instanceof MagickGeometry) {
      ...
      geometry = new MagickGeometry(
        geometryOrWidth,
        widthOrGravityOrBackgroundColor,
      );
    }
    ...
    Exception.use((exception) => {
      _withString(geometry.toString(), (geometryPtr) => {
        const instance = ImageMagick._api._MagickImage_Extent(
          this._instance,
          geometryPtr,
          gravity,
          exception.ptr,
  ...
  getAttribute(name: string): string | null {
    return Exception.use((exception) => {
      return _withString(name, (namePtr) => {
        const value = ImageMagick._api._MagickImage_GetAttribute(
          this._instance,
          namePtr,
          exception.ptr,
        );
  ...
  getProfile(name: string): IImageProfile | null {
    return _withString(name, (namePtr) => {
      const value = ImageMagick._api._MagickImage_GetProfile(
        this._instance,
        namePtr,
      );
  ...
  level(
    channelsOrBlackPoint: Channels | Percentage,
    blackPointOrWhitePoint: Percentage,
    whitePointPercentageOrGamma?: Percentage | number,
    gammaOrUndefined?: number,
  ): void {
    ...
    Exception.usePointer((exception) => {
      ImageMagick._api._MagickImage_Level(
        this._instance,
        blackPoint.toDouble(),
  ...
  modulate(
    brightness: Percentage,
    saturationOrUndefined?: Percentage,
    hueOrUndefined?: Percentage,
  ): void {
    const saturation = this.valueOrDefault(
      saturationOrUndefined,
      new Percentage(100),
    );
    const hue = this.valueOrDefault(hueOrUndefined, new Percentage(100));
    Exception.usePointer((exception) => {
      const modulate =
        `${brightness.toDouble()}/${saturation.toDouble()}/${hue.toDouble()}`;
      _withString(modulate, (modulatePtr) => {
        ImageMagick._api._MagickImage_Modulate(
          this._instance,
          modulatePtr,
          exception,
        );
  ...
  motionBlur(radius: number, sigma: number, angle: number): void {
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_MotionBlur(
        this._instance,
        radius,
        sigma,
        angle,
  ...
  morphology(settings: MorphologySettings): void {
    Exception.use((exception) => {
      _withString(settings.kernel, (kernelPtr) => {
        const instance = ImageMagick._api._MagickImage_Morphology(
          this._instance,
          settings.method,
          kernelPtr,
          settings.channels,
  ...
  oilPaint(radiusOrUndefined?: number): void {
    ...
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_OilPaint(
        this._instance,
        radius,
        sigma,
        exception.ptr,
  ...
  ping(
    ...
    settingsOrUndefined?: MagickReadSettings,
  ): void {
  ...
  read(array: ByteArray, settings?: MagickReadSettings): void;
  read(
    fileNameOrArrayOrColor: string | ByteArray | MagickColor,
    settingsOrWidthOrUndefined?: MagickReadSettings | number,
    heightOrUndefined?: number,
  ): void {
    this.readOrPing(
  ...
  resize(
    ...
    const geometry = typeof widthOrGeometry === "number"
      ? new MagickGeometry(widthOrGeometry, heightOrUndefined as number)
      : widthOrGeometry;
    Exception.use((exception) => {
      _withString(geometry.toString(), (geometryPtr) => {
        const instance = ImageMagick._api._MagickImage_Resize(
          this._instance,
          geometryPtr,
          exception.ptr,
        );
  ...
  rotate(degrees: number): void {
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_Rotate(
        this._instance,
        degrees,
        exception.ptr,
      );
  ...
  separate<TReturnType>(
    ...
    return Exception.use((exception) => {
      ...
      const images = ImageMagick._api._MagickImage_Separate(
        this._instance,
        channels,
        exception.ptr,
      );
  ...
  setArtifact(name: string, value: string | boolean): void {
    ...
    _withString(name, (namePtr) => {
      _withString(strValue, (valuePtr) => {
        ImageMagick._api._MagickImage_SetArtifact(
          this._instance,
          namePtr,
          valuePtr,
        );
    ...
    Exception.use((exception) => {
      _withString(name, (namePtr) => {
        _withString(value, (valuePtr) => {
          ImageMagick._api._MagickImage_SetAttribute(
            this._instance,
            namePtr,
            valuePtr,
            exception.ptr,
    ...
    Exception.use((exception) => {
      _withString(name, (namePtr) => {
        _withByteArray(data, (dataPtr) => {
          ImageMagick._api._MagickImage_SetProfile(
            this._instance,
            namePtr,
            dataPtr,
            data.byteLength,
  ...
  setWriteMask(image: IMagickImage): void {
    Exception.usePointer((exception) => {
      ImageMagick._api._MagickImage_SetWriteMask(
        this._instance,
        image._instance,
        exception,
      );
  ...
  sharpen(
    ...
    Exception.use((exception) => {
      const instance = ImageMagick._api._MagickImage_Sharpen(
        this._instance,
        radius,
        sigma,
        channels,
  ...
  splice(geometry: MagickGeometry): void {
    MagickRectangle.use(this, geometry, (geometryPtr) => {
      Exception.use((exception) => {
        const instance = ImageMagick._api._MagickImage_Splice(
          this._instance,
          geometryPtr,
          exception.ptr,
        );
